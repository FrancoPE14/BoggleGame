...
function buildInitialPlayers(): LobbyPlayer[] {
    ...
    return [
        { username, ready: false, isCurrentUser: true },
        { username: "Player2", ready: true, isCurrentUser: false },
        { username: "Player3", ready: false, isCurrentUser: false },
    ];
}
...
export default function LobbyPage(): React.JSX.Element {
    ...
    /**
     * Toggles the current user's ready status.
     */
    const toggleReady = (): void => {
        setPlayers((prev) =>
            prev.map((p) =>
                p.isCurrentUser ? { ...p, ready: !p.ready } : p
            )
        );
    };
        ...
        <div className="flex flex-col items-center justify-start min-h-screen bg-amber-50 px-4 py-8">
            ...
            <div className="flex flex-col items-center gap-3 w-full max-w-2xl">
                <button
                    type="button"
                    onClick={toggleReady}
                    className="px-6 py-2 rounded-lg border-2 border-amber-400 text-amber-700 font-medium hover:bg-amber-100 transition-colors"
                    aria-label={
                        currentUser?.ready
